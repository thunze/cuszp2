 ...
 * @param   nbEle           original data size (number of doubleing point)
 ...
 * @param   cmpBytes        compressed data (host pointer)
 * @param   nbEle           reconstructed data size (number of doubleing point)
 * @param   cmpSize         compressed data size (number of unsigned char)
 * @param   errorBound      user-defined error bound
 * *********************************************************************** */
void GSZ_decompress_hostptr(double* decData, unsigned char* cmpBytes, size_t nbEle, size_t cmpSize, double errorBound)
{
    ...
    cudaMemcpy(decData, d_decData, sizeof(double)*pad_nbEle, cudaMemcpyDeviceToHost);
 ...
 * *********************************************************************** */
void GSZ_compress_deviceptr_plain(double* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, double errorBound, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * cmp_chunk - 1) / (bsize * cmp_chunk);
 ...
 * @param   stream          CUDA stream for executing compression kernel
 * *********************************************************************** */
void GSZ_decompress_deviceptr_plain(double* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, double errorBound, cudaStream_t stream)
{
    // Data blocking.
    int bsize = dec_tblock_size;
    int gsize = (nbEle + bsize * dec_chunk - 1) / (bsize * dec_chunk);
 ...
 * @param   stream          CUDA stream for executing compression kernel
 * *********************************************************************** */
void GSZ_compress_deviceptr_outlier(double* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, double errorBound, cudaStream_t stream)
{
    // Data blocking.
    int bsize = cmp_tblock_size;
    int gsize = (nbEle + bsize * cmp_chunk - 1) / (bsize * cmp_chunk);
 ...
 * @param   stream          CUDA stream for executing compression kernel
 * *********************************************************************** */
void GSZ_decompress_deviceptr_outlier(double* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, double errorBound, cudaStream_t stream)
{
    // Data blocking.
    int bsize = dec_tblock_size;
    int gsize = (nbEle + bsize * dec_chunk - 1) / (bsize * dec_chunk);
