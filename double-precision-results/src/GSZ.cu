...
__global__ void GSZ_compress_kernel_outlier(const double* const __restrict__ oriData, 
        ...
        if(fixed_rate[j])
        {
            ...
            if(vec_ofs==0)
            {
                ...
                for(int i=0; i<fixed_rate[j]; i++)
                {
                    ...
                    // If using outlier fixed-length encoding, then first element is set as 0.
            ...
            else if(vec_ofs==1)
            {
                ...
                // Assign sign information and block bit-shuffle, padding for vectorization.
                tmp_char.x = 0;
                tmp_char.y = 0;
                tmp_char.z = 0;
                tmp_char.w = 0;
...
__global__ void GSZ_decompress_kernel_outlier(double* const __restrict__ decData, 
    ...
    for(int j=0; j<block_num; j++)
    {
        ...
        if(fixed_rate[j])
        {
            ...
            else if(vec_ofs==1)
            {
                ...
                // Retrieve sign information for one block and reverse block bit-shuffle, padding for vectorization.
                tmp_char = reinterpret_cast<const uchar4*>(cmpData)[cmp_byte_ofs/4];
...
__global__ void GSZ_compress_kernel_plain(const double* const __restrict__ oriData, 
                                         unsigned char* const __restrict__ cmpData, 
                                         volatile unsigned int* const __restrict__ cmpOffset, 
                                         volatile unsigned int* const __restrict__ locOffset, 
                                         volatile int* const __restrict__ flag, 
...
__global__ void GSZ_decompress_kernel_plain(double* const __restrict__ decData, 
                                            const unsigned char* const __restrict__ cmpData, 
                                            volatile unsigned int* const __restrict__ cmpOffset, 
                                            volatile unsigned int* const __restrict__ locOffset, 
                                            volatile int* const __restrict__ flag, 
...
__device__ inline int quantization_src_backup(double data, double recipPrecision)
{
    double dataRecip = data*recipPrecision;
    int s = dataRecip>=-0.5f?0:1;
    return (int)(dataRecip+0.5f) - s;
