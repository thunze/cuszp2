...
int main(int argc, char* argv[])
{
    ...
    for(int i=0; i<3; i++)
    {
        GSZ_compress_deviceptr_plain(d_oriData, d_cmpBytes, nbEle, &cmpSize, errorBound, stream);
    }
    ...
    // GSZ decompression.
    timer_GPU.StartCounter(); // set timer
    GSZ_decompress_deviceptr_plain(d_decData, d_cmpBytes, nbEle, cmpSize, errorBound, stream);
